...
const UpSetAxis = /*!#__PURE__*/ React__default['default'].memo(function UpSetAxis({ size, style, data }) {
    ...
    return (React__default['default'].createElement("g", null,
        ...
        React__default['default'].createElement("g", { transform: `translate(${size.cs.x},${size.cs.y})`, "data-upset": "csaxis" },
        ...
        React__default['default'].createElement("g", { transform: `translate(${size.sets.x},${size.sets.y})`, "data-upset": "setaxis" },
...
const UpSetJS = /*!#__PURE__*/ React__default['default'].forwardRef(function UpSetJS(props, ref) {
    ...
    const styleInfo = React.useMemo(() => deriveStyleDependent(p.theme, p.styles, p.classNames, p.combinationName, p.combinationNameAxisOffset, p.setName, p.setNameAxisOffset, styleId, p.barLabelOffset, p.selectionColor, p.emptySelection, p.title, p.description, p.tooltips, p.setLabelAlignment), [
        p.theme,
        p.styles,
        p.classNames,
        p.barLabelOffset,
  ...
  `;
    ...
    return (React__default['default'].createElement("svg", { id: p.id, className: clsx(`root-${styleId}`, p.className), style: p.style, width: p.width, height: p.height, ref: ref, viewBox: `0 0 ${p.width} ${p.height}`, "data-theme": (_a = p.theme) !== null && _a !== void 0 ? _a : 'light', "data-selection": selectionName ? selectionName : undefined },
        React__default['default'].createElement("style", null, rules),
...
function VennArcSliceSelection({ slice, d, i, data, style, elemOverlap, selected, selectionName, h, queries, size, fill, qs, }) {
    ...
    return (React__default['default'].createElement("g", null,
        ...
        React__default['default'].createElement("text", { x: slice.text.x, y: slice.text.y, className: clsx(`${d.type === 'set' ? 'set' : 'value'}TextStyle-${style.id}`, `pnone-${style.id}`
...
function useCreateCommon(p) {
    ...
    const sizeInfo = React.useMemo(() => deriveVennSizeDependent(p.width, p.height, p.padding, p.id), [
        p.width,
        p.height,
        p.padding,
        p.id,
    ...
    const h = useHandler(p);
    const qs = React__default['default'].useMemo(() => queries.map((q) => model.queryOverlap(q, 'intersection', p.toElemKey)), [
        queries,
        p.toElemKey,
    ]);
    const rulesHelper = baseRules(styleId, p, p.fontFamily, fontSizes);
...
const VennDiagram = /*!#__PURE__*/ React__default['default'].forwardRef(function VennDiagram(props, ref) {
    ...
    const dataInfo = React.useMemo(() => deriveVennDataDependent(p.sets, p.combinations, size, p.layout, p.valueFormat, p.toKey, p.toElemKey, p.id), [p.sets, p.combinations, size, p.valueFormat, p.toKey, p.toElemKey, p.id, p.layout]);
 ...
 * Copyright (c) 2021 Samuel Gratzl <sam@sgratzl.com>
 */
const KMapQueries$1 = /*!#__PURE__*/ React__default['default'].memo(function KMapQueries$1({ data, style, hasHover, secondary, queries, }) {
...
function KMapSelection({ data, style, selection, hasHover, }) {
    ...
    const selectionOverlap = generateSelectionOverlap(selection, data.overlapGuesser, data.toElemKey);
    const selectionName = generateSelectionName(selection);
    return (React__default['default'].createElement("g", { className: hasHover ? `pnone-${style.id}` : undefined }, (selection || empty) && (React__default['default'].createElement(KMapQueries, { data: data, style: style, empty: empty && !selection, elemOverlap: selectionOverlap, suffix: `Selection-${style.id}`, tooltip: hasHover ? undefined : selectionName }))));
}
...
const KarnaughMap = /*!#__PURE__*/ React__default['default'].forwardRef(function KarnaughMap(props, ref) {
    ...
    const style = React.useMemo(() => deriveStyleDependent$1(p.theme, p.styles, p.classNames, p.combinationName, p.combinationNameAxisOffset, styleId, p.barLabelOffset, p.selectionColor, p.emptySelection, p.title, p.description, p.tooltips), [
        p.theme,
        p.styles,
        p.classNames,
        p.combinationName,
    ...
    const size = React.useMemo(() => deriveVennSizeDependent(p.width, p.height, p.padding, p.id), [
        p.width,
        p.height,
        p.padding,
        p.id,
  ...
  `;
    ...
    return (React__default['default'].createElement(SVGWrapper, { rules: rules, style: style, size: size, p: p, data: data, tRef: ref, selectionName: selectionName, h: h, exportChart: exportChart },
        ...
        React__default['default'].createElement(KMapQueries$1, { style: style, data: data, hasHover: h.hasHover, queries: queries, secondary: h.hasHover || selection != null })));
});
